...
// function averageDollarValue (value) {
//     const f = fp.flowRight(_average, fp.map(item => item.dollar_value))
//     return f(value)
// }
// 代码优化
...
//     const f = fp.flowRight(fp.map(_underscore), fp.map(fp.lowerCase), fp.map(item => item.name))
//     return f(value)
// }
// 代码优化
let sanitizeNames = fp.flowRight(fp.map(_underscore), fp.map(fp.lowerCase), fp.map(item => item.name))
// 2.打印测试
console.log(sanitizeNames(cars)) // 最终返回 ['ferrari_ff', 'spyker_c_12_zagato', 'jaguar_xkr_s', 'audi_r_8', 'aston_martin_one_7', 'pagani_huayara']